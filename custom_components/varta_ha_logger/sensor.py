...
# Configuration values that are useful diagnostically but should not flood the entity list.
SKIP_ROOTS = {'params'}
...
class VartaDynamicSensor(CoordinatorEntity, SensorEntity):
    def __init__(self, coordinator, entry_id, path):
        ...
        meta = KNOWN.get(path)
        self._attr_name = meta[0] if meta else _friendly(path)
        if meta:
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for path, _ in _flatten(coordinator.data):
        if not path or path[0] in SKIP_ROOTS:
            continue
        # Avoid duplicate raw SunSpec values when a clean summary sensor exists.
        if path == ('summary','kaco_connected'):
            continue
        key = _slug(path)
